    ...
    public static void mainMenu(List<Vehicule> vehiculeList){
        ...
        switch (action){
            case "1":
                ...
                Print.printCharacteristicsVehicule(vehiculeList.get(Integer.parseInt(characteristicsCar)));
                break;

            case "2":
                System.out.println("Which car?");
                String characteristicsMotor = scanner.nextLine();
                Print.printCharacteristicsMotor(vehiculeList.get(Integer.parseInt(characteristicsMotor)).getMotor());
                break;

            case "3":
                System.out.println("Which car?");
                String characteristicsMark = scanner.nextLine();
                Print.printCharacteristicsMark(vehiculeList.get(Integer.parseInt(characteristicsMark)).getMark());
                break;

            case "4":
                System.out.println("Which car?");
                String checkReady = scanner.nextLine();
                Factory.checkReady(vehiculeList.get(Integer.parseInt(checkReady)));
                break;
